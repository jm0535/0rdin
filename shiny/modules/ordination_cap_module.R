    ...
    div(class = "cap-workflow",
      ...
      div(class = "horizontal-split",
        div(class = "plot-panel", style = "max-width: 100%; overflow: hidden;",
          ...
          plotOutput(ns("cap_plot"), width = "100%", height = "500px"),
...
cap_server <- function(id, data, env_data) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      session$sendCustomMessage(type = "updateGroupingVar", message = list(moduleId = "cap-", choices = choices_list))
    })
    ...
    output$cap_plot <- renderPlot({  
      ...
      p <- generate_constrained_plot(cap_result(), plot_defaults, grouping_var, show_env_vectors = show_env_vectors)
      ...
      if (!is.null(grouping_var) && !is.null(plot_ellipse_type)) {
        ...
        p <- add_ordination_ellipses(p, cap_result(), grouping_var, plot_ellipse_type, ellipse_level)
      }
