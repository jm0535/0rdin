...
rda_server <- function(id, data, env_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$rda_plot <- renderPlot({  
      ...
      plot_show_species <- plot_defaults$show_species
      plot_plot_type <- plot_defaults$plot_type
      ...
      # Determine whether to show vectors
      ...
      p <- generate_constrained_plot(rda_result(), plot_defaults, grouping_var, show_env_vectors = show_env_vectors)
      ...
      if (!is.null(grouping_var) && !is.null(plot_ellipse_type)) {
        ...
        p <- add_ordination_ellipses(p, rda_result(), grouping_var, plot_ellipse_type, ellipse_level)
      }
