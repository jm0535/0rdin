  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$dbrda_plot <- renderPlot({  
      ...
      if(input$equal_aspect) {
        plot(dbrda_result(), type = "none", main = "db-RDA Triplot", font.main = 2, scaling = as.numeric(input$scaling))
      ...
      } else plot(dbrda_result(), type = "none", main = "db-RDA Triplot", font.main = 2, scaling = as.numeric(input$scaling))
      
      if(input$show_grid) grid(col = grid_color, lty = 1)
      points(dbrda_result(), display = "sites", pch = as.numeric(input$point_shape),
             bg = input$point_color, cex = input$point_size, col = fg_color, lwd = input$point_lwd)
      if(input$show_labels) text(dbrda_result(), display = "sites", cex = input$label_size,
                                 pos = as.numeric(input$label_pos), col = fg_color)
      
      # Add environmental vectors
      text(dbrda_result(), display = "bp", col = if(is_dark) "#e67e22" else "#d35400", cex = input$label_size * 1.1)
    })
    ...
    output$export_plot <- downloadHandler(
      filename = function() paste0("dbrda_plot_", Sys.Date(), ".", input$export_format),
