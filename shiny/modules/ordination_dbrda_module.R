...
dbrda_server <- function(id, data, env_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$dbrda_plot <- renderPlot({  
      ...
      points(dbrda_result(), display = "sites", pch = as.numeric(plot_defaults$point_shape),
             bg = plot_defaults$point_color, cex = plot_defaults$point_size, col = fg_color, lwd = plot_defaults$point_lwd)
      if(plot_defaults$show_labels) text(dbrda_result(), display = "sites", cex = plot_defaults$label_size,
                                 pos = as.numeric(plot_defaults$label_pos), col = fg_color)
      
      # Add environmental vectors
      text(dbrda_result(), display = "bp", col = if(is_dark) "#e67e22" else "#d35400", cex = plot_defaults$label_size * 1.1)
    })
